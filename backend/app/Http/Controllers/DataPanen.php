...
class Mahasiswa extends Controller
{
    ...
    function viewData()
    {
        ...
        if (count($this->model->viewData()) == 0) {
            $data = [];
            $error = 1;
            $message = "Data Panen Ikan Tidak Ditemukan !";
        }
        // jika data panen ikan ada / tersedia
        else {
            // ambil method "viewData" dari model "MDataPanen"
            $data = $this->model->viewData();
            $error = 0;
            $message = "";
        }

        return response(["panen" => $data, "error" => $error, "message" => $message], http_response_code());
    }

    // buat fungsi untuk pencarian data
    function searchData($keyword)
    {
        // isi nilai variabel "data" dari fungsi "searchData" dari model "MDataPanen" sesuai dengan isi parameter "keyword"
        $data = $this->model->searchData($keyword);

        // kembalikan nilai variabel "result" ke dalam object "databibit"
        return response(["panen" => $data], http_response_code());
    }
    ...
    function saveData(Request $req)
    {
        ...
        // jika kode belum ada
            ...
            // proses simpan data
            $this->model->saveData($kode, $jenis, $jumlah, $tanggal);
    ...
    {
        // cek apakah data panen ikan (kode) tersedia/tidak pada model checkData
        // jika data tersedia
    ...
    {
        // isi nilai variabel "result" dari fungsi "detailData" dari model "MdataPanen" sesuai dengan isi parameter "id"
        $data = $this->model->detailData($kode);

        // kembalikan nilai variabel "result" ke dalam object "panen"
        return response(["panen" => $data], http_response_code());
    }
    ...
    function editData($kode_lama, Request $req)
    {
        ...
        if (count($this->model->checkEditData($kode_lama, $kode)) == 0) {
            ...
            $this->model->editData($kode, $jenis, $jumlah, $tanggal, $kode_lama);
